...
async def get_tournament_bootstrap_snapshot(db: AsyncSession, tournament_id: int) -> TournamentBootstrapSnapshot:
    tournament = await get_tournament(db, tournament_id)
    brackets = await get_tournament_brackets(db, tournament_id, sorted_brackets=True)
    bracket_matches = await get_matches_for_tournament_full(db, tournament_id)
